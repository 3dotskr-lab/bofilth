...
-- 누구나(로그인 없이) 등록한 초록집 삭제 가능 (초록집 삭제 버튼에 필요)
create policy "Anon can delete speaker-photos"
on storage.objects for delete
to anon
using ( bucket_id = 'speaker-photos' );
```

> 위 4개 정책을 모두 실행해야 사진/초록집 등록·수정·삭제가 전부 정상 동작합니다.
> DELETE 정책이 없으면 "초록집 삭제" 버튼을 눌렀을 때 "삭제 권한이 아직 설정되지
> 않았습니다" 라는 오류만 표시됩니다.
...
- **초록집 등록**: "초록집 등록" 버튼 클릭 → pdf/doc/docx/ppt/pptx/hwp/hwpx/zip 파일 선택 (최대 50MB). 등록된
  파일명이 버튼 아래 표시되며, 다시 올리면 기존 파일이 새 파일로 자동 교체됩니다. 등록된 초록집
  옆의 ✕ 버튼으로 삭제할 수 있습니다 (삭제하려면 위 SQL의 DELETE 정책이 먼저 적용돼 있어야 합니다).
- **전체 다운로드**: 우측 상단 "전체 사진 다운로드" / "전체 초록집 다운로드" 버튼 → 비밀번호
  `bofilth2026` 입력 → 등록된 파일을 모두 zip으로 묶어 다운로드합니다. (비밀번호는 페이지 소스에
  평문으로 저장하지 않고 SHA-256 해시로만 비교합니다. 다만 정적 페이지 특성상 완전한 보안은 아니며,
  가벼운 접근 제한 용도입니다.)
